...
>
  <div class="max-w-7xl mx-auto text-center">
    ...
    <p class="text-xl text-gray-600 dark:text-gray-300 max-w-2xl mx-auto mb-8">
      Connect Employee Envoy with {fullCatalogCount} business applications to automate
      your employee lifecycle management.
    </p>

    <!-- Stats -->
    <div class="flex flex-wrap justify-center gap-6 mb-8">
      <div
        class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 rounded-full shadow-sm border border-gray-200 dark:border-slate-700"
      >
        <div
        ...
        <span class="text-sm font-medium text-gray-900 dark:text-white"
          >{fullCatalogCount} Integrations</span
        >
      </div>
      <div
        class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 rounded-full shadow-sm border border-gray-200 dark:border-slate-700"
...
<!-- Search & Filters -->
<section
  class="sticky top-16 z-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm border-b border-gray-200 dark:border-slate-800 py-4 px-4 sm:px-6 lg:px-8"
>
  <div class="max-w-7xl mx-auto">
    ...
    <div class="flex items-center justify-between mt-4">
      <p class="text-sm text-gray-600 dark:text-gray-400">
        Showing <span class="font-medium text-gray-900 dark:text-white"
          >{filteredIntegrations().length}</span
        > integrations
      </p>
      {#if hasActiveFilters}
        <button
  ...
  <div class="max-w-7xl mx-auto">
    {#if filteredIntegrations().length === 0}
      <div class="text-center py-16">
        <div
          class="w-16 h-16 rounded-full bg-gray-100 dark:bg-slate-800 flex items-center justify-center mx-auto mb-4"
        >
    ...
    {:else}
      ...
      >
        {#each filteredIntegrations() as integration (integration.id)}
          <div
            class="group p-5 rounded-xl bg-slate-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 hover:border-amber-300 dark:hover:border-amber-600 hover:shadow-lg transition-all"
          >
            <!-- Header -->
            <div class="flex items-start justify-between mb-3">
              <div>
                <h3
                  class="font-semibold text-gray-900 dark:text-white group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors"
                >
                  {integration.name}
                </h3>
              ...
              <span
                class="text-xs px-2 py-1 rounded-full font-medium {getTierColor(
                  integration.tier,
                )}"
              >
            ...
            <div class="flex flex-wrap gap-2 mb-3">
              {#if integration.scimSupport}
                <span
                  class="inline-flex items-center gap-1 text-xs px-2 py-1 rounded bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400"
                >
              ...
              {#if integration.ssoSupport}
                <span
                  class="inline-flex items-center gap-1 text-xs px-2 py-1 rounded bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400"
                >
                  <Key class="w-3 h-3" />
                  SSO
                </span>
              {/if}
              <span
                class="inline-flex items-center gap-1 text-xs px-2 py-1 rounded bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300"
              >
                <Clock class="w-3 h-3" />
...
>
  <div class="max-w-4xl mx-auto text-center">
    <h2 class="text-4xl font-bold text-white mb-4">Don't see your app?</h2>
    <p class="text-xl text-amber-100 mb-8">
      We're constantly adding new integrations. Contact us to request a specific
      integration or learn about our custom API options.
    </p>
